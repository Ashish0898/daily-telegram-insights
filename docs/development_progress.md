...
12. **Auth0 Authentication & Administrative Dashboard**:
    ...
    - Switched to **database-driven email verification** by checking the Auth0 email against the `email` column in the `allowed_users` table in Supabase. It uses a new `is_email_admin` helper in [src/db.py](file:///mnt/c/Users/Ashish/Downloads/playground/daily-telegram-insights/src/db.py) with case-insensitive `ilike` query matching.
